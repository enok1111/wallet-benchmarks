...
## Status: Blocked on Testnet tXTM Funds

### What's Done

**PR #3** (`feat: complete wallet benchmark harness`) is open, compiles, and contains all 9 scenarios (B0-S7) across 3 wallet modes. The one remaining requirement before the baseline run is testnet funding.
...
Our built v5.3.1 Esmeralda binaries at `tari/target/release/` are functional and could be saved for repeated use. For even longer-term storage:
- `tari/target/release/minotari_node` (79 MB)
- `tari/target/release/minotari_console_wallet` (53 MB)
...
However, the wallet-benchmarks harness uses gRPC (port 18142) for wallet ↔ base node communication, which requires a local base node. The RPC is HTTP-only, not gRPC.

#### 4. `test-faucet` Tool ❌

The `tari-project/test-faucet` repo generates 4,000 × 10 T UTXOs for a custom genesis block on a **local devnet**. However, the code depends on `tari_core::transactions` module paths that were refactored out in v5.3.1 (moved to `tari_transaction_components`). The tool is incompatible with the current code structure and would require significant adaptation.
...
- A public HTTP RPC is available (`https://rpc.esmeralda.tari.com`) but only offers HTTP REST, not gRPC
...
2. **PR review first** — Request a maintainer review of PR #3 as-is (without baseline results). The harness logic is complete; the baseline JSON can be produced once access to funded wallets is available.
...
3. **Alternative: Use the `minotari` CLI for manual funding** — If a community member provides tXTM:
   ...
   - Run the harness with `seed_words_*` configured
